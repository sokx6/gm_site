...
func (r *CommentRepository) Create(comment *model.Comment) error {
	...
	result, err := r.db.Exec(
		`INSERT INTO comments (image_id, user_id, content, created_at)
		 VALUES (?, ?, ?, ?)`,
		comment.ImageID, comment.UserID, comment.Content, comment.CreatedAt,
	)
	if err != nil {
...
func (r *CommentRepository) FindByImageID(imageID int64, page, limit int) ([]model.Comment, int, error) {
	...
	var total int
	err := r.db.QueryRow(
		`SELECT COUNT(*) FROM comments WHERE image_id = ?`, imageID,
	).Scan(&total)
	if err != nil {
	...
	for rows.Next() {
		var c model.Comment
		if err := rows.Scan(&c.ID, &c.ImageID, &c.UserID, &c.Content, &c.CreatedAt); err != nil {
